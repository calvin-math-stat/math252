...
* **program**: A list of **instructions**. Each instruction has 5 parts:
    ...
    * output
        ...
        * `<direction>`: left, right, or stay  (Some flavors of Turing machine do not allow stay.)
...
There are several websites that provide Turing machine simulators. Here are three:
...
* <https://calvin-turing.netlify.app/>
...
In style of <https://calvin-turing.netlify.app/>
```
ATM       // A Turing Machine
Example 1 // A name for the machine
0 1       // Input alphabet
0 1 B     // Tape alphabet -- use B for blnk
1         // Number of tapes
...
We will say that a Turing machine $M$ accepts (or recognizes) a string $x$ (by state) 
if on input $x$ it halts in an accepting state. 
...
:::{#exr-design-tm-for-a-regular-language}
...
:::{#exr-design-tm-for-palindromes}
Design a Turing machine that recognizes palindromes.  (A palindrome is a string
that reads the same forwards and backwards.  Examples: 010, 1001, 110010011.)
:::
...
1 // number of tracks on tape 0
2 // tape 0 is 2-way infinite
searchR // the initial state
accept // final state
searchR ) searchL ] l 
searchR B noR B l 
searchR * searchR * r 
searchL ( searchR [ r 
searchL B reject B r    // no ( for some )
searchL * searchL * l 
noR     B accept B r    // no ( either
noR     ( reject ( r    // extra (
noR     * noR * l 
end
```
...
Write a Turing machine that neatly computes the function $f(x) = 2x$ in
unary.
:::
...
to build a new machine $K$ that works as follows.
    ...
    b. Now run machine $H$ with two modifications.
        ...
        i. change all accepting states to the state LOOP and and add the instruction
            ...
            ```
            LOOP * LOOP * r  // this will cause the machine to move right forever.
            ```
...
Let's consider $H(\enc{K}\#\enc{K}$). (We're going to show that $H$ gets the wrong answer.)
