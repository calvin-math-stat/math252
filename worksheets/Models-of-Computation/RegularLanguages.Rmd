...
* If $\bm{A}$ and $\bm{B}$ are regular expressions representing languages $A$ and $B$, then
    * $(\bm{AB})$ represents $AB$
    * $(\bm{A \cup B})$ represents $A \cup B$ \hfill 
    [Note: This is often written `(A|B)` in programming languages.]
    * $\bm{A}^*$ represents $A^*$  \hfill 
...
3. Show that if $a \in I$, then the langauge $\{a \}$ is recognized by an NFA.

4. Show that if $A$ and $B$ are each recognized by an NFA, then $AB$ is, too.

    [Hint: Let $N_A$ and $N_B$ be the NFAs that recognize $A$ and $B$.  How can you use them to
    build a new NFA that recognizes $AB$?]

5. Show that if $A$ and $B$ are each recognized by an NFA, then $A \cup B$ is, too.

    [Hint: Let $N_A$ and $N_B$ be the NFAs that recognize $A$ and $B$.  How can you use them to
    build a new NFA that recognizes $A \cup B$?]
    
6. Show that if $A$ is recognized by an NFA, then $A^*$ is, too.

    [Hint: Let $N_A$ be the NFA that recognizes $A$ How can you use it to
    build a new NFA that recognizes $A \cup B$?]
...
that recognizes $\bm{1}^* \cup \bm{01}$.  Do this two ways:
  
    a. Following the steps outlined in 1--6.
    b. Any way you like, but using fewer states than in part a.  
    c. What is the smallest (fewest states) NFA you can find that recognizes $\bm{1}^* \cup \bm{01}$?
...
### Solution to problem 9 on previous page
