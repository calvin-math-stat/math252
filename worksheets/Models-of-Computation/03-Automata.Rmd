...
with a graph.  Create a labeled, directed graph with the following properties:
    ...
    * Accepting states are circled a second time.  (We could use shape or color or something else to make it clear which states are accepting states, but double circling is easy.)
...
15. Which is harder: to convince someone that a string *is* accepted by an NFA or that it *is not*
...
19. Given an NFA $N$, is it always possible to create a DFA $M$ that recognizes
...
20. One of these two algorithms is super easy. Which one? What is the algorithm?

21. The other direction is more interesting. Let's see if we can figure out a 
...
22. Now convert the NFA $N_1$ into an equivalent DFA.  Here's $N_1$ again.

```{r, echo = FALSE, out.width = "50%", fig.align = "center"}
knitr::include_graphics("../images/DFA-Fig-12-3-7.png")
```

23. If an NFA has $n$ states and we use this method to convert it to a DFA,
