...
we need to specify the actiion of the machine for any tape symbol and state combination.
...
<!-- b. <https://turingmachinesimulator.com/>  -->
...
6. Design a Turing machine that recognizes 
$(\mybold{0} \cup \mybold{1})\mybold{1}(\mybold{0} \cup \mybold{1})^*$.
Is every regular language recognized by a Turing machine?  How do you know?
...
7. Design a Turing machine that recognizes
$\{0^n1^n \mid n \ge 0\}$.  
...
8. Design a Turing machine that has input
alphabet $\{$ `(`, `)`, `0`, `1`, `+` $\}$ 
...
9. Design a Turing machine that adds in unary. Use the input alphabet
$\{1,+\}$.
...
10. Modify your previous machine so that it computes neatly. What is the 
...
2. Construct a Turing Machine that recognizes the same language
...
3. Write a 2-tape Turing machine to do parenthesis matching. It should run much
faster than a 1-tape Turing machine for the same task. How will you take
advantage of the second tape?

4. Give a high level description of how to convert a Turing 
machine with 2-way infinite tape into a Turing machine with a 
1-way infinite tape.

5. Give a high level description of how to convert a 2-tape 
...
6. Suppose $H(\enc{K}\#\enc{K})$ accepts. 
...
7. Suppose $H(\enc{K}\#\enc{K})$ rejects. 
...
8. Combine these results to explain why the Halting Problem is not 
Turing Computable.
