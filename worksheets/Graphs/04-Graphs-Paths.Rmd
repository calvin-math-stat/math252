...
If $f : G \to H$ is an isomorphism, then $f$ maps paths in $G$ to paths in $H$. (Why?)
This can be useful for finding and isomorphism between $G$ and $H$ or for showing that there
is no isomorphism.

4. Answer some questions about the two graphs below.
 
    a. Compute the degreee sequences for each graph.
    
    b. Find the shortest and longest simple paths from $u_1$ to $u_4$.
...
5. Answer some questions about the two graphs below.
 
    a. Compute the degreee sequences for each graph.
    
    b. Find the shortest and longest simple paths from $u_1$ to $u_2$.
 ...
 * A connected component of a subgraph $H$ of a graph $G$ is maximal connected subgraph.
   That is, $H$ must be a subgraph of $G$, $H$ be connected, but $H$ cannot be a subgraph 
   of a larger connected subgraph of $G$.
 
 6. Draw a directed graph with 6 vertices that is weakly connected but not strongly connected.
 
 7. Draw an undirected graph with two connected components.
...
Euler paths are named after Leonid Euler who posed the following famous problem about the 
...
the paper and without repeating anything.

 8. Which kind of path is this puzzle asking for?

 9. Which of these can you trace in this manner?
...
```
 ...
 10. Create a graph that represents the Königsberg Bridge Puzzle (so it can be traced 
...
\vfill
 ...
 11. Can you trace the Königsberg Bridge graph?
...
A **planar graph** is a graph that it can be drawn in 
the plane without any edges crossing.

 12. Show that $K_4$ and $Q_3$ are both planar graphs.
...
```
 ...
 13. Add dots showing vertices so that each graph is a planar graph.

 14. Fill in the table below for the graphs above.
...
15. **The Blue Prince's eighth bridge.**
...
16. **The Red Prince's ninth bridge.**
